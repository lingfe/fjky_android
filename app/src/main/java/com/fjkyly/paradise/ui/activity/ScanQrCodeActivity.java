...
public class ScanQrCodeActivity extends BaseActivity {
    ...
    public void initEvent() {
        // 设置扫描结果的监听
        setScannerListener(new OnRxScanerListener() {
            @Override
            public void onSuccess(String type, Result result) {
                RxToast.success(result.getText());
            }
...
//            decodeFormats.addAll(DATA_MATRIX_FORMATS);
    ...
    public void btn(View view) {
        ...
        if (viewId == R.id.top_mask) {
            ...
            finish();
        } else if (viewId == R.id.top_open_picture) {
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (resultCode == Activity.RESULT_OK) {
            ...
            try {
                ...
                LogUtils.d(GsonUtils.toJson(photo));
                // 开始对图像资源解码
                Result rawResult = RxQrBarTool.decodeFromPhoto(photo);
                LogUtils.d(rawResult);
                if (rawResult != null) {
                    ...
                    if (mScannerListener == null) {
                        RxToast.error("二维码识别失败");
                    } else {
                        mScannerListener.onFail("From to Picture", "二维码识别失败");
                    }
                }
            } catch (Exception e) {
                e.printStackTrace();
    ...
    public void handleDecode(Result result) {
        ...
        LogUtils.v(this, "二维码/条形码 扫描结果 ======> " + resultStr);
        //处理扫描结果
        if (TextUtils.isEmpty(resultStr)) {
            resultStr = "";
        }
        Intent intent = new Intent();
        intent.putExtra(ConstantsKt.QR_SCAN_CONTENT, resultStr);
        setResult(Activity.RESULT_OK, intent);
        finish();
        // Pattern pattern = Pattern.compile("[hH][tT]{2}[pP]://|[hH][tT]{2}[pP][sS]://");
        // Matcher matcher = pattern.matcher(resultStr);
        // resultStr = matcher.find() && matcher.start() == 0 ? resultStr : "http://" + resultStr;
        // ToastKt.simpleToast(String.format("识别到了以下内容：%s", resultStr));
    }
